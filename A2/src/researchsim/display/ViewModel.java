...
public class ViewModel {
    ...
    public void updateScenarioLog() {
        Logger currentLog = ScenarioManager.getInstance().getScenario().getLog();

        if (! currentLog.getEvents().isEmpty()) {
            StringJoiner logEvents = new StringJoiner(System.lineSeparator());
            for (int i = 0; i < currentLog.getEvents().size(); i++) {
                logEvents.add(currentLog.getEvents().get(i).toString());
            }
            scenarioLogText.setValue(currentLog.toString());

            StringJoiner statEvents = new StringJoiner(System.lineSeparator());
            statEvents.add(String.format("Entities Collected: %d",
                    currentLog.getEntitiesCollected()));
            statEvents.add(String.format("Tiles traversed: %d",
                    currentLog.getTilesTraversed()));
            statEvents.add(String.format("Points Earned: %d",
                    currentLog.getPointsEarned()));
            scenarioStatisticsText.setValue(statEvents.toString());
        }
    }
    ...
    public void saveAs(Writer scenarioWriter) throws IOException {
        BufferedWriter bufferedWriter = new BufferedWriter(scenarioWriter);
        bufferedWriter.write(ScenarioManager.getInstance().getScenario().encode());
